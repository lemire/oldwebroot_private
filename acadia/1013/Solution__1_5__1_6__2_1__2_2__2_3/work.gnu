...
#    
# set terminal postscript eps enhanced color dashed defaultplex "Helvetica" 14
# set output 'question2_2_20b.eps'
set noclip points
set clip one
set noclip two
set bar 1.000000
...
set tmargin -1
set locale "C"
f(x) = x<-1 ? 2- x : x < 1 ? x : (x-1)*(x-1)
g(x) = (tan(x)-x)/(x*x*x)
power(x,y) = exp(y * log(x))
plot [-0.000001:0.000001] g(x) notitle
#    EOF
