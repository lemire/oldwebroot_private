...
\usepackage{color}
 ...
 \pdfinfo{
            ...
            /Subject 	(By using temporary placeholders on a dense grid, we generalize the 4-point dyadic cubic Deslauriers-Dubuc scheme. These generalized stationary linear interpolatory subdivision schemes are 4-point dyadic schemes and thus have the same support as the Deslauriers-Dubuc scheme; they also have the same computational complexity and memory requirements. However, interpolated values require 2 steps to stabilize in general as they are first interpolated on a coarse scale through a tetradic filter and then refined on a finer scale using a dyadic filter. Some of these generalized schemes have fundamental functions with fast decays and thus are more local and it also possible to reproduce polynomials of degree 4. This work has applications in CAGD and wavelet theory..)
...
sur l'intervalle obtenue par un schéma d'interpolation itérative,
Ann. Sci. Math. Québec 23 (1999), no. 1, 37-48.
