...
\ifpdf
 \usepackage{thumbpdf}
 \usepackage[pdftex,plainpages=false,urlcolor=black,colorlinks=false]{hyperref}
 \pdfinfo{
...
subdivision schemes \cite{Han2001,Plonka,HeilColella} all characterized
by the idea that each node should store a vector instead of a single
value.

Adding extra nodes to improve an interpolation scheme is not a new
approach and has been used to make spline interpolation local \cite{DuGe}.
However, doubling the number of nodes doubles the memory requirements.
On the other hand, dyadic subdivision scheme double their memory usage
at each step. Hence, we can choose to use one step earlier the upcoming
extra storage space without any cost. These new intermediate nodes
or placeholders can then be used to record a coarse scale guess (using
a tetradic filter) which we can later combine with a finer scale interpolation
(using a dyadic filter). Because the placeholders are used as predictors,
it is reasonable to expect that the new schemes will be as local as
usual subdivision schemes. These schemes are said to be {}``high
resolution'' because we no longer consider only the next finer scale,
but actually the next two finer scales; alternatively, we could describe
these algorithms as {}``two-step subdivision schemes''. Whereas
...
at finer scales. Whereas a vector subdivision scheme would store two
or more values per node, we store only one value, but we allow it
change over time.
...
(see Table \ref{cap:Comparaison-between-some}). While there exists
$5-$point quartic subdivision schemes, none of them is as local as
the presented HRS.
...
by proving that some of these schemes are smooth ($C^{1}$).
...
\caption{\label{cap:Comparaison-between-some}Comparison between some $4-$point
iterative interpolation schemes. The quartic HRS scheme is slightly
less local because it requires initialization by a one-step $5-$point
scheme.}
\end{table}
...
\caption{Diagrams of $4-$point subdivision schemes in the dyadic (left) and
...
scheme is $2N-$point if $\gamma _{l}=0$ for $|l|\geq Nb$. The fundamental
...
;
\item interpolate quartile point values by the corresponding cubic Lagrange
polynomial: \[
...
grid, we must apply a dyadic subdivision scheme as an initialization
step: since HRS schemes can be seen as multistep subdivision schemes,
it is not surprising that they require an initialization step. For
...
We say it is interpolatory if it satisfies $y_{j+1,Mbk}=y_{j,Mk}$
and it is $2N-$point if $\gamma _{l}^{(m)}=0$ for $|l|\geq MNb$
and $m=1,...,M$. For $b=M=2$ the general equation \ref{hrgeneral}
...
$\gamma _{l}^{(m)}=0$ for $|l|\geq 4N$ and $m=1,2$. It should be
noted that for an algorithm based on HRS scheme to be interpolatory,
...
lemma. Note that while this algorithm is as local as the corresponding
Deslauriers-Dubuc subdivision scheme in the sense that the fundamental
function has support $[-3,3]$. 
...
Rolle's theorem, given any 4 points $\xi _{1}$,$\xi _{2}$,$\xi _{3}$,
and $\xi _{4}$, the corresponding cubic polynomial $p_{Lagrange3}$
approximates $p_{4}$ with error \begin{eqnarray*}
p_{4}(x)-p_{Lagrange3}(x) & = & \frac{p_{4}'(\xi )\left(x-\xi _{1}\right)\left(x-\xi _{2}\right)\left(x-\xi _{3}\right)\left(x-\xi _{4}\right)}{4!}\\
 & = & a_{4}\left(x-\xi _{1}\right)\left(x-\xi _{2}\right)\left(x-\xi _{3}\right)\left(x-\xi _{4}\right)
\end{eqnarray*}
for some $\xi $. In other words, the error depends only on $a_{4}$
and the geometry of the sample points $\xi _{i}$ with respect to
$x$. This makes the task of canceling out the errors given more two
different approximating cubic polynomials convenient as we shall see.

Suppose that for some $j$, $y_{j,2k}=p_{4}\left(x_{j,2k}\right)$
and $y_{j-1,k}=p_{4}\left(x_{j-1,k}\right)\, \forall k\in \Z $. We
can write $y_{j+1,4r+2}$ for any $r\in \Z $ in terms of this initial
data ($y_{j}$ and $y_{j-1}$) by substituting equation \ref{hrswidguess}
...
\end{eqnarray*}
we can compute both sums in equation \ref{upcomingrichardson} explicitly:
\begin{eqnarray}
...
\end{eqnarray*}
Incidentally, these two results can be checked using the Generalized
Rolle's theorem. Hence, setting $\alpha =-3/32$ in equation \ref{upcomingrichardson},
we get \[
...
For practical applications, we wish to initialize HRS schemes with
...
$4-$point HRS algorithm reproduces quartic polynomials when $\alpha =-3/32$.
\end{prop}
...
we see that only subdivision schemes using $5$ points can interpolate
quartic polynomials. Starting with $y_{0,k}=\delta _{k,0}\, \forall k\in \Z $,
in the best possible case, a $5-$point subdivision scheme gives an
interpolation function having a support of size $8$. For example,
...
are possible because in the presented HRS scheme, the placeholders
...
P^{j+1}(z)=\Phi _{1}(z)P^{j}\left(z^{2}\right)+\Phi _{2}(z)P^{j}\left(-z^{2}\right).\]
 The equation of symbols for the $4-$point HRS scheme is (see equation
...
\end{thm}
\begin{proof}
See the proof of theorem 3.4 \cite{Dyn} or section 4.2 of \cite{DynLevin}
...
1987.
\bibitem{DynLevin}N. Dyn, D. Levin, Subdivision schemes in geometric modelling, Acta
Numerica, 12, pp. 1-72, 2002
\bibitem{Han2001}B. Han, Approximation Properties and Construction of Hermite Interpolants
and Biorthogonal Multiwavelets, Journal of Approximation Theory, 110
no.1, pp. 18-53, 2001.
...
\bibitem{HeilColella}C. Heil and D. Colella, Matrix refinement equations and subdivision
schemes, J. Fourier Anal. Appl., 2, pp. 363-377, 1996.
...
M2An Math. Model. Numer. Anal. 33, pp. 55-65, 1999.
\bibitem{Plonka}G. Plonka, Approximation order provided by refinable function vectors,
Constr. Approx. 13, pp. 221-244, 1997.\end{thebibliography}
