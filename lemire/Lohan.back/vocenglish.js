...
function getEnglishVoice() {
    	  ...
    	  var englishvoices = voices.filter(function(voice) { 
    	   return voice.lang.lastIndexOf('en',0) === 0; });
...
function showBoxes() {
        ...
        var mycontent = "<ol '>"
        for (i = 0; i < howmany; i++) {
            var motanglais = mots[i][0];
            mycontent = mycontent + '<li style="margin-bottom:0;margin-top:0;">'+motanglais+' <input type="text" name="lname" id="rep' + i + '" onfocus=\'javascript:speak(' + i  + ');\' onkeypress=\'javascript:if(event.keyCode==13){checkval(' + i + ');}\'/> <input type="submit" id="but' + i + '" value="Soumettre" onclick=\'javascript:checkval(' + i + ');\' /> <span id="indice' + i + '" style="font-style: italic; color: grey;"></span></li>';
        }
...
function speak(i) {
     try {
      ...
      // Set the attributes.
    	...
    	msg.lang = 'fr';
    	msg.rate = 0.9;
    	var englishvoice = getEnglishVoice();
    	if(englishvoice) {
